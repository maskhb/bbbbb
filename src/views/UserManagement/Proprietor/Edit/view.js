...
export default class View extends AddComponent {
  ...
  componentDidMount() {
    const { roomOwnerId } = this.props.match.params || {};
    const { dispatch } = this.props;
    this.fetchDetail(roomOwnerId);
    dispatch({
      type: "tag/tagDocType",
      payload: {}
    });
  ...
  // 获取详情
  fetchDetail = async roomOwnerId => {
    const { dispatch } = this.props;
    return await dispatch({
      type: "proprietor/roomOwnerDetail",
      payload: {
        roomOwnerId
      }
    });
    ...
    return (
      <PageHeaderLayout>
        <Card title="基本信息" bordered={false}>
          ...
          <Form.Item label="生日" {...formItemLayout}>
            {getFieldDecorator("birthday", {
              initialValue: roomOwnerDetail?.birthday
                ? moment(+roomOwnerDetail?.birthday)
                : ""
        ...
        <Card title="关联房间" bordered={false}>
          {roomOwnerDetail?.roomOwnerId ? (
            <LinkTable {...this.props} editting={true} />
          ) : (
            <span>
              <Icon type="loading" />
              数据加载中...
